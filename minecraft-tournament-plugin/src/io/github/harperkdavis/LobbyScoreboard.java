...
public class LobbyScoreboard extends BukkitRunnable {
    ...
    public void updateTopList() {
        ...
        int[] playerScores = new int[main.playerList.size()];

        for (int i = 0; i < playerScores.length; i++) {
            playerScores[i] = main.playerList.get(i).score;
        }

        Arrays.sort(playerScores);

        for (int score : playerScores) {
            for (ScoredPlayer sp : main.playerList) {
        ...
        for (ScoredPlayer sp : sortedPlayerList) {
            ArmorStand as = (ArmorStand) world.spawnEntity(plyldb.add(h), EntityType.ARMOR_STAND);
            as.setVisible(false);
            as.setCustomNameVisible(true);
        ...
        ArmorStand topPlayerLDB = (ArmorStand) world.spawnEntity(teamldb.add(h), EntityType.ARMOR_STAND);
